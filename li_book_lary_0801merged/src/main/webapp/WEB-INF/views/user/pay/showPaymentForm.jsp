    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (table) {
            for (var i = 1; i < table.rows.length; i++) {
                ...
                var inputElement = table.rows[i].cells[3].querySelector("input[name='number']");
			...
			if(rsp.success){
				...
				$.ajax({
					...
					data : JSON.stringify({
						...
						// book_isbn : book_isbn,
						// book_qry : book_qry,
						saving_point : saving_point // 적립포인트 - 사용포인트
					}),
					dataType : "json"
				}).done(function(data){
					console.log(data);
...
<br><br>
<div align="center"><h3 class="text-custom">주문 상품</h3></div>
...
<table id="order_table">
	...
	<c:if test="${empty listBasket}">
		<tr>
			...
			<td><input type="number" name="number" min="0" max="500" step="1" value="1" onchange="changeTotal()"></td>
			<td id="toc-content">${bookVO.book_isbn}</td>
		</tr>
	</c:if>
	<c:if test="${not empty listBasket}">
		<c:forEach var="imsi" items="${listBasket}">
			<tr>
				<td>${imsi.book_name}</td>
				<td>${imsi.book_price}</td>
				<td>${imsi.book_point}</td>
				<td><input type="number" name="number" min="0" max="500" step="1" value="${imsi.basket_qry}" onchange="changeTotal()"></td>
...
<div align="center">
	<h4 class="text-custom">배송선택</h4><br>
	<input type="radio" name="order_info" value="일반택배"> 일반택배
	<input type="radio" name="order_info" value="편의점택배"> 인터넷택배
</div>

<hr>
<br>

<div>사용 포인트 : <input type="text" name="usePoint" id="usePoint" size="25" placeholder="사용할 포인트를 입력하세요." value="0" onchange="changeTotal()"></div>
<div>사용가능 포인트 : ${sesseionScope.memberVO.member_point}</div>
</div>
<br>
<hr>
...
<input type="hidden" name="member_point" id="member_point" value="${sessionScope.memberVO.member_point}"> 
<input type="hidden" name="member_name" id="member_name" value="${sessionScope.memberVO.member_name}"> 
<input type="hidden" name="member_email" id="member_email" value="${sessionScope.memberVO.member_email}"> 
<input type="hidden" name="member_address" id="member_address" value="${sessionScope.memberVO.member_address}"> 
<input type="hidden" name="member_num" id="member_num" value="${sessionScope.memberVO.member_num}"> 
<input type="hidden" name="member_tel" id="member_tel" value="${sessionScope.memberVO.member_tel}">
<input type="hidden" name="book_isbn" id="book_isbn" value="${bookVO.book_isbn}">
<input type="hidden" name="book_qry" id="book_qry" value="${bookVO.book_qry}">
