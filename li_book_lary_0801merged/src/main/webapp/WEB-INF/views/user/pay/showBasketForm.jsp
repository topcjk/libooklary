	...
	var memberNum = '${sessionScope.memberVO.member_num}';
		...
		function renderBaslist(baslistData) {
			...
			for (var i = 0; i < baslistData.length; i++) {
				...
				memberNumCell.setAttribute('value', 'memberNum'
						+ basketRow.member_num);
			...
			var firstBasketList = document.getElementById('firstBasketList');
			if (firstBasketList) {
				firstBasketList.remove();
			}
...
-->
	...
	<table id="firstBasketList">
		...
		<c:forEach var="baslist" items="${baslist }">
			<tr>
				...
				<td>${baslist.book_name }<input type=hidden value="${baslist.book_name }" id="book_name">
				</td>
				<td>${baslist.book_price }<input type=hidden value="${baslist.book_price}" id="book_price">
				</td>
				<td>${baslist.basket_qry}<input type=hidden value="${baslist.basket_qry}" id="basket_qry">
					<button type=button class=btn1 onclick="addBasketQry('${baslist.book_isbn}', '${sessionScope.memberVO.member_num}')">+</button>
					<button type=button class=btn2 onclick="subsBasketQry('${baslist.book_isbn}', '${sessionScope.memberVO.member_num}', '${baslist.basket_qry }')">-</button>
				</td>
				<td>
					${baslist.counted }
					<input type=hidden value="${baslist.counted }" id="counted"> 
					<input type=hidden value="${sessionScope.memberVO.member_num}" id="member_num"> 
					<input type=hidden value="${baslist.book_isbn }" id="book_isbn">
					<input type=hidden value="${baslist.book_point }" id="book_point">
				</td>
			</tr>
		</c:forEach>
	</table>
	...
	<div id="buttonsContainer" style="margin-right:1200px;">
		<button type=button class=btn1 id="delbut" name="delbut" onclick="basketSelect()"  >선택항목 지우기</button>
	
		<span id="newButtonContainer"></span>

	
	<!-- 세션값 필요 -->
		<button type=button class=btn1 onclick="location.href='clearBasket.do?member_num=${sessionScope.memberVO.member_num}'">장바구니 비우기</button>
	</div>
	...
	<button type=button class=btn1 onclick="location.href='goPayment.do?member_num=${sessionScope.memberVO.member_num}'">결제창으로</button>
